...
const Project = ({ portfolioOwnerId, isEditable }) => {
  ...
  const projectFilterUserId = projects.filter(
    (project) => project.user_id === portfolioOwnerId
  );
        ...
        <Col>
          <Card className="mb-3">
            <Card.Body>
              ...
              {projectFilterUserId &&
                projectFilterUserId.map((item, index) => (
                  <ProjectCard
                    key={item.id}
                    project={item}
              ...
              {isEditing && <AddProject setIsEditing={setIsEditing} />}
            </Card.Body>
          </Card>
        </Col>
      </Row>
